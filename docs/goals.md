...
The manuscript review is the full-book pass, in either frame. Three reviewers run in parallel (Claude, Codex, Pi), plus an optional Pi adversary, then a cross-review matrix where each reviewer rebuts the others, then a synthesis pass that produces ranked issues with stable IDs.

The delta review is the same panel pointed at one round of changes. Reviewers get the whole manuscript for context and the added and modified scenes as the target, and answer whether the changes did what the author said they were for and whether they're good on their own terms.
...
1. Honest feedback over polite feedback. The reviewer never reports to the author. In the publication frame it's a consultant advising the agent who already represents the author; in the craft frame it's a consulting reader advising the author's developmental editor. Neither is an encouraging friend. The rejection pass and the adversary exist to counter alignment bias in both.
...
3. Measured against the right thing. The publication frame measures against a saleable finished book, which is right when a publisher is in the picture and wrong otherwise: it produces notes about acquisition editors, comparable titles, and the author's drafting cadence for a book written on weekends for its own sake. The craft frame measures against the author's stated intent, keeps the bar at professional craft, and puts the market and the author's process out of scope. The frame is a choice of prompts, not a different pipeline.

4. Calibrated to the actual draft stage. Reviewers default to assuming they're reading a finished book. That produces useless verdicts like "the arc doesn't resolve" when the author has only drafted Act 1. The critic injects an explicit stage block, either hand-written (`stage.md`) or auto-derived from project frontmatter, so reviewers know what they're seeing.

5. Scoped context per task. The full-manuscript review gets the manuscript, the stage block, the style guide, and past-review context, and nothing else: manuscript reviewers read as readers, without the worldbuilding bible. A chapter review inlines only the Codex entries for entities in that chapter. A close-read on a scene inlines only that scene's frontmatter entities plus the style guide. Reviewers don't get to wander.

6. Independent reviewers, then forced engagement. The three primary reviewers run in parallel with no knowledge of each other. Then the cross-review matrix forces each one to rebut the others. This surfaces real disagreements (which the synthesis flags) and stops one reviewer's framing from dominating.
